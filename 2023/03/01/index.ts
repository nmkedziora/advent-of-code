...
function checkNumber({
  ...
  // matches a string that contains any symbols other than numbers and period symbols (".")
  const regExp = /[^0-9.]+/;

  const current =
    startIndex > 0
      ? currentRow.substring(startIndex, endIndex + 1)
      : currentRow.substring(0, endIndex + 1);
  const previous = previousRow
    ? startIndex > 0
      ? previousRow.substring(startIndex, endIndex + 1)
      : previousRow.substring(0, endIndex + 1)
    : "";
  const next = nextRow
    ? startIndex > 0
      ? nextRow.substring(startIndex, endIndex + 1)
      : nextRow.substring(0, endIndex + 1)
    : "";

  console.log({ current, next, previous });

  // check if part of any row (current, previous, next)
  // contains any symbols other than number and periods
  if (current.match(regExp) || previous.match(regExp) || next.match(regExp)) {
    // if yes: it IS a part number
    return true;
  } else {
    // if no: it is NOT a part number
    return false;
  }
}
